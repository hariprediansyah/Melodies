...
const Playlist = forwardRef(({ playlist, onSelectSong, selectedSong, onPlaySong, currentSong }, ref) => {
  return (
    <div className='row-span-9 grid grid-rows-12 bg-black bg-opacity-50 border border-white/10 p-6 rounded-lg h-full text-white'>
      <h2 className='row-span-1 text-3xl font-bold mb-4'>Playlist ({playlist.length})</h2>
        ...
        {playlist.length === 0 ? (
          ...
          playlist.map((song, index) => {
            ...
            return (
              <div
                key={index}
                className={`p-2 rounded-md flex text-2xl justify-between items-center group transition-all cursor-pointer 
