...
export default function RoomManagement({ onAdd, onEdit }) {
  ...
  const renderCell = (col, room, idx) => {
    ...
    if (col.key === 'description') return <span className='truncate max-w-xs'>{room.description}</span>
    if (col.key === 'status') return <span className={`font-semibold ${statusColor[room.status]}`}>{room.status}</span>
    if (col.key === 'actions')
      return (
        <div className='flex gap-2'>
          <button className='p-2 rounded hover:bg-gray-700' onClick={() => onEdit(room.id)}>
          ...
          <button className='p-2 rounded hover:bg-gray-700' onClick={() => handleDelete(room.id)}>
            <svg xmlns='http://www.w3.org/2000/svg' width='24' height='24' viewBox='0 0 24 24' fill='none'>
              <path
